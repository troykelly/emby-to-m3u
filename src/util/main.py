    ...
    try:
        with temp_file as f:
            ...
            for track in new_tracks:
                ...
                if (
                    os.getenv("AZURACAST_HOST")
                    and os.getenv("AZURACAST_API_KEY")
                    and os.getenv("AZURACAST_STATIONID")
                ):
        ...
        try:
            ...
            if e.errno == 36:  # Filename too long
                logger.warning("Filename too long, shortening and retrying: %s", filename)
...
def shorten_filename(name: str, max_length: int = 255) -> str:
    ...
    excess_length = total_length - max_length + len(parts) - 1  # Include separators
    truncation_len = excess_length // len(parts)

    shortened_parts: List[str] = [part[: len(part) - truncation_len] for part in parts]
